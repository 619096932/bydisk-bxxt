...
<head>
	...
	<title>管理员订单处理 - 报修平台Ver3.0.3</title>
								...
								<div class="card-main">
									<nav class="margin-top-no tab-nav">
										<ul class="nav nav-justified">
											<li class="active">
												<a class="waves-attach waves-effect" data-toggle="tab" href="#doc_tab_example_1" aria-expanded="true">未处理</a>
											</li>
											<li class="">
												<a class="waves-attach waves-effect" data-toggle="tab" href="#doc_tab_example_2" aria-expanded="false">正在处理</a>
											</li>
											<li class="">
												<a class="waves-attach waves-effect" data-toggle="tab" href="#doc_tab_example_3" aria-expanded="false">完成</a>
											</li>
										</ul>
									...
									<div class="card-inner">
										<div class="tab-content">
											...
											<?php
											$stmt_none=$conn->prepare("SELECT * from avada where NOW=?");
											$stmt_none->bind_param("s", $top_none);
											$stmt_none->execute();
											//$stmt_user->bind_result();
											$result_none=$stmt_none->get_result();
											while($rows_none = mysqli_fetch_array($result_none))
											echo '
												<div class="card">
													<div class="card-main">
														<div class="card-inner">
															<p class="card-heading">'.$rows_none['DATATIME'].'</p>
															<p>
																姓名:'.$rows_none['NAME'].'<br>
																地址：'.$rows_none['AREA'].$rows_none['ROOM'].'<br>
																手机：'.$rows_none['PHONE'].'<br>
																类型：'.$rows_none['TYPE'].'<br>
																锐捷：'.$rows_none['RGID'].'<br>
																描述：'.$rows_none['Description'].'<br>
																上交：'.$rows_none['REG_TOPID'].'<br>
															</p>
														</div>
														<div class="card-action">
															<div class="card-action-btn pull-left">
																<a class="btn btn-flat waves-attach waves-button waves-effect" href="engine.php?action=regrun&key='.$rows_none[DATATIME].'&id='.$username.'"><span class="icon">check</span>&nbsp;YES</a>
																<a class="btn btn-flat waves-attach waves-button waves-effect" href="engine.php?action=reg_cancel&key='.$rows_none[DATATIME].'&id='.$username.'"><span class="icon">clear</span>&nbsp;NO</a>
															</div>
														</div>
													</div>
												</div>
											...
											<?php
											$stmt_wor=$conn->prepare("SELECT * from avada where NOW=? ORDER BY JDTIME DESC");
											$stmt_wor->bind_param("s", $top_run);
											$stmt_wor->execute();
											...
											echo '
												...
												<div class="card">
													<div class="card-main">
														...
														<div class="card-action">
															<div class="card-action-btn pull-left">
																<a class="btn btn-flat waves-attach waves-button waves-effect" href="engine.php?action=regdone&key='.$rows_wor[DATATIME].'&id='.$username.'"><span class="icon">check</span>&nbsp;YES</a>
															</div>
														</div>
													</div>
												</div>
											...
											<?php
											$stmt_black=$conn->prepare("SELECT * from avada where NOW=? ORDER BY JDTIME DESC");
											$stmt_black->bind_param("s", $top_done);
											$stmt_black->execute();
